...
export default  function () {
    ...
    (function () {
        ...
        textCenter($svg.find('#zuorichanliang .num').text(Number(dayData).toFixed(1)));
        textCenter($svg.find('#dangyuechanliang .num').text(Number(monthData).toFixed(1)));
        textCenter($svg.find('#shishichanliang .num').text(Number(nowData).toFixed(1)));
    })();
    // 取向、无取向
    (function () {
        const qx = getArr('report1','A6');
